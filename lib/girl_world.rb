...
def high_school_ratio(country)
	HTTParty.get("http://api.worldbank.org/countries/#{country}/indicators/SE.ENR.PRSC.FM.ZS?&format=json")
end 

def hign_school_ration(country, start_year, end_year)
	HTTParty.get("http://api.worldbank.org/countries/#{country}/indicators/SE.ENR.PRSC.FM.ZS?date=#{start_year}:#{end_year}")
end 
